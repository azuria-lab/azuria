...
import { type BillingInterval, corsHeaders, type EdgeFunctionResponse, type MercadoPagoPreference, type PlanId, PLANS } from '../_shared/types.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const preference: MercadoPagoPreference = await mercadoPagoRequest('/checkout/preferences', {
      method: 'POST',
      body: JSON.stringify({
        items: [
          {
            id: planId,
            title: `${plan.name} - ${billingInterval === 'monthly' ? 'Mensal' : 'Anual'}`,
    ...
    const response: EdgeFunctionResponse<{ checkoutUrl: string; preferenceId: string }> = {
      success: true,
      data: {
