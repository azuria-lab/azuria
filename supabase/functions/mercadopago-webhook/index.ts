...
import { corsHeaders, type EdgeFunctionResponse, type MercadoPagoPayment, type MercadoPagoWebhookNotification } from '../_shared/types.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (notification.type !== 'payment') {
      return new Response(JSON.stringify({ success: true, message: 'Notification type not handled' }), {
    ...
    const payment: MercadoPagoPayment = await mercadoPagoRequest(`/v1/payments/${notification.data.id}`, {
      method: 'GET',
    });
    ...
    return new Response(JSON.stringify(response), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200, // Retornar 200 mesmo em erro para não reprocessar
    });
  }
});
