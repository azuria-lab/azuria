...
serve(async req => {
  ...
  try {
    ...
    if (!planId || !billingInterval) {
      throw new Error(
        'Missing required parameters: planId and billingInterval'
      );
    }
    ...
    const PRODUCTS = {
      essencial: {
        monthly: {
          ...
          description:
            'Acesso completo às funcionalidades essenciais da Azuria',
          quantity: 1,
          price: 5900,
        },
        annual: {
    ...
    console.error('Error in abacatepay-create-billing:', error);
